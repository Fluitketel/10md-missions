...
_obj removeAction (_obj getVariable ["Arsenal_actionID", -1]);
terminate (_obj getVariable ["Arsenal_restrictionHandle", scriptNull]);

_slotsArray = _obj getVariable ["Arsenal_currentSlots", []];
_index = [_slotsArray, {_x select 0 isEqualTo _target}] call Arsenal_fnc_conditionalIndex;
...
_obj setVariable ["Arsenal_specNames", _specNames];
_obj setVariable ["Arsenal_specItems", _specItems];

[_specIndex, _obj] call Arsenal_fnc_updateLocalArsenal;
...
_obj setVariable ["Arsenal_currentSlots", _slotsArray, true];

_objArray = player getVariable ["Arsenal_objArray", []];
_index = [_objArray, {_x select 0 isEqualTo _obj && _x select 1 isEqualTo _target}] call Arsenal_fnc_conditionalIndex;
if (_index isEqualTo -1) then {
	_objArray pushBack [_obj, _target, _specIndex];
} else {
	_objArray set [_index, [_obj, _target, _specIndex]];
};
player setVariable ["Arsenal_objArray", _objArray, true];
